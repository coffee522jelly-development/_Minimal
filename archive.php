...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6 <?php echo esc_attr( $config['grid_gap_class'] ); ?> items-start">
    <div class="<?php echo esc_attr( $config['main_cols_class'] ); ?> <?php echo esc_attr( $config['main_order'] ); ?>">
        <header class="mb-12 border-b border-base-200 pb-8">
            <?php the_archive_title( '<h1 class="text-4xl font-extrabold mb-4">', '</h1>' ); the_archive_description( '<div class="opacity-60">', '</div>' ); ?>
        </header>
        ...
        <?php else : ?>
            <div class="alert">
                ...
                <span>No posts found in this archive.</span>
            </div>
        <?php endif; ?>
    </div>
